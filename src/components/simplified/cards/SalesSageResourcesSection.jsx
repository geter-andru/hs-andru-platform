...
import { Target, User, Brain, TrendingUp, Eye, X, Star, Copy, Check } from 'lucide-react';
...
const ResourceModal = ({ resource, onClose }) => {
  ...
  const copyToClipboard = async () => {
    try {
      ...
      let copyText = `${resource.title}\n${'='.repeat(resource.title.length)}\n\n`;
      
      if (resource.content.confidence_score) {
        copyText += `Confidence Score: ${resource.content.confidence_score}/10\n\n`;
      }
      
      if (resource.content.text) {
        copyText += `${resource.content.text}\n\n`;
      }
      
      if (resource.content.data && Object.keys(resource.content.data).length > 0) {
        copyText += `DETAILED ANALYSIS\n${'-'.repeat(17)}\n\n`;
        Object.entries(resource.content.data).forEach(([key, value]) => {
          if (value) {
            const formattedKey = key.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            copyText += `${formattedKey}:\n${value}\n\n`;
          }
        });
      }
